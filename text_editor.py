...
# vertical colum = frame
# relief = 3d appearane, raised = will raise the element, bd = border
...
# stick to north south, es = east south
    frame.grid(row=0, column=0, sticky="ns")

# x here is an argument
    window.bind("<Control-s>", lambda x: save_file(window, text_edit))
    window.bind("<Control-o>", lambda x: open_file(window, text_edit))
# continually run the window 
# untill user hits the x
    window.mainloop()
main()